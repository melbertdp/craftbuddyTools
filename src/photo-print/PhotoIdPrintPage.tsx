...
import { PAPER_SIZES, PHOTO_SIZES, orientedPaper, type SheetCell } from "./layout";

const DPI = 300;
const PHOTO_WIDTH_MM = 31;
const PHOTO_HEIGHT_MM = 41;
const PHOTO_WIDTH_PX = Math.round((PHOTO_WIDTH_MM / 25.4) * DPI);
const PHOTO_HEIGHT_PX = Math.round((PHOTO_HEIGHT_MM / 25.4) * DPI);
const MARGIN_MM = 4;
const GAP_MM = 2.5;
...
function maxCopies(widthMm: number, heightMm: number) {
  const columns = Math.max(0, Math.floor((widthMm - MARGIN_MM * 2 + GAP_MM) / (PHOTO_WIDTH_MM + GAP_MM)));
  const rows = Math.max(0, Math.floor((heightMm - MARGIN_MM * 2 + GAP_MM) / (PHOTO_HEIGHT_MM + GAP_MM)));
  return columns * rows;
}

function cellsForCopies(widthMm: number, heightMm: number, copies: number): SheetCell[] {
  const columns = Math.max(1, Math.floor((widthMm - MARGIN_MM * 2 + GAP_MM) / (PHOTO_WIDTH_MM + GAP_MM)));
  const placedColumns = Math.min(copies, columns);
  const totalWidth = placedColumns * PHOTO_WIDTH_MM + Math.max(0, placedColumns - 1) * GAP_MM;
  const rows = Math.ceil(copies / columns);
  const totalHeight = rows * PHOTO_HEIGHT_MM + Math.max(0, rows - 1) * GAP_MM;
  const startX = (widthMm - totalWidth) / 2;
  const startY = (heightMm - totalHeight) / 2;
  return Array.from({ length: copies }, (_, index) => ({
    id: `passport-${index}`,
    photoTypeId: "passport",
    xMm: startX + (index % columns) * (PHOTO_WIDTH_MM + GAP_MM),
    yMm: startY + Math.floor(index / columns) * (PHOTO_HEIGHT_MM + GAP_MM),
...
export default function PhotoIdPrintPage() {
  ...
  const maximum = maxCopies(paper.widthMm, paper.heightMm);
  const cells = cellsForCopies(paper.widthMm, paper.heightMm, Math.min(copies, maximum));
  ...
  React.useEffect(() => {
    if (!bgRemove || !crop || activeStep !== 2) {
      setProcessedCrop(null);
      setBgStatus("");
      if (activeStep !== 2 && bgRemove) setBgRemove(false);
      return;
    }
    let cancelled = false;
  ...
  function downloadPng() {
    ...
    cells.forEach((cell) => { context.drawImage(finalCanvas, cell.xMm * scale, cell.yMm * scale, cell.widthMm * scale, cell.heightMm * scale); context.strokeStyle = "#000"; context.lineWidth = 2; context.strokeRect(cell.xMm * scale, cell.yMm * scale, cell.widthMm * scale, cell.heightMm * scale); });
    const link = document.createElement("a"); link.download = `passport_${copies}x.png`; link.href = sheet.toDataURL("image/png"); link.click();
  }
  ...
  async function downloadPdf() {
    ...
    pdf.save(`passport_${copies}x.pdf`);
  }
  ...
  function printPreview() {
    ...
    const scale = DPI / 25.4; cells.forEach((cell) => { context.drawImage(finalCanvas, cell.xMm * scale, cell.yMm * scale, cell.widthMm * scale, cell.heightMm * scale); context.strokeStyle = "#000"; context.strokeRect(cell.xMm * scale, cell.yMm * scale, cell.widthMm * scale, cell.heightMm * scale); });
  ...
  return <main className="mx-auto w-full max-w-[1400px] px-4 py-8 text-ink md:px-7">
    ...
    <div className="grid gap-5 lg:grid-cols-[380px_minmax(0,1fr)]">
      ...
      <section className="rounded border border-line bg-[#f4f6f9] p-5"><div className="mb-4 flex flex-wrap gap-4 text-xs text-muted-foreground"><span><b className="text-foreground">{copies}</b> photos</span><span>{basePaper.label}</span><span>{paper.widthMm.toFixed(1)} × {paper.heightMm.toFixed(1)} mm</span></div><div className="flex flex-wrap items-start justify-center gap-8"><div className="text-center"><p className="mb-2 text-xs font-bold">Passport preview</p><div className="relative mx-auto overflow-hidden rounded border-2 border-ink bg-white" style={{ width: 170, height: Math.round(170 * PHOTO_HEIGHT_MM / PHOTO_WIDTH_MM) }}>{finalUrl && <img src={finalUrl} alt="Passport preview" className="h-full w-full object-cover" />}</div><p className="mt-2 text-[11px] text-muted-foreground">{PHOTO_WIDTH_MM / 10} × {PHOTO_HEIGHT_MM / 10} cm</p></div><div className="text-center"><p className="mb-2 text-xs font-bold">Full print sheet</p><div className="overflow-auto rounded bg-white p-3 shadow-sm"><div className="relative mx-auto bg-white" style={{ width: `${paper.widthMm}mm`, height: `${paper.heightMm}mm` }}>{cells.map((cell) => <div key={cell.id} className="absolute overflow-hidden border border-black" style={{ left: `${cell.xMm}mm`, top: `${cell.yMm}mm`, width: `${cell.widthMm}mm`, height: `${cell.heightMm}mm` }}>{finalUrl && <img src={finalUrl} alt="" className="h-full w-full object-cover" />}</div>)}</div></div><p className="mt-2 text-[11px] text-muted-foreground">{copies} photos · drag position is controlled by the crop tool</p></div></div></section>
    </div>
  </main>;
}
