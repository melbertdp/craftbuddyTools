...
const shapes = ["Square", "Rounded Square", "Circle", "Diamond", "Triangle", "Hexagon", "Star", "Burst", "Plus", "Horizontal Pill", "Heart", "Chevron", "Blob", "Dot"];
const frames = ["Square", "Rounded Square", "Circle", "Heavy Rounded", "Octagon", "Diamond"];
const pupils = ["Square", "Rounded Square", "Circle", "Diamond", "Star", "Burst", "Plus", "Heart", "Chevron"];
...
function shapeNode(shape: string, x: number, y: number, size: number, rotation = 0): ReactNode {
  ...
  if (["Star", "Burst"].includes(shape)) {
    const points = Array.from({ length: 10 }, (_, i) => { const angle = i * Math.PI * 2 / 10 - Math.PI / 2; const radius = i % 2 ? size * .43 : size * .18; return `${cx + Math.cos(angle) * radius},${cy + Math.sin(angle) * radius}`; }).join(" ");
    return <polygon transform={transform} points={points} />;
  }
  if (["Plus", "Chevron"].includes(shape)) return <path transform={transform} d={shape === "Plus" ? `M${x + size * .35} ${y + a}h${size * .3}v${size * .25}h${size * .25}v${size * .3}h-${size * .25}v${size * .25}h-${size * .3}v-${size * .25}H${x + a}v-${size * .3}h${size * .25}z` : `M${x + a} ${y + size * .25}L${cx} ${y + size * .75}L${x + size - a} ${y + size * .25}v${size * .25}L${cx} ${y + size - a}L${x + a} ${y + size * .5}z`} />;
...
export default function QRDesigner() {
  ...
  const control = (label: string, key: "thickness" | "jitter" | "quiet" | "logoSize") => <label className="flex flex-col gap-1.5 text-xs font-semibold text-muted"><span>{label}</span><div className="flex items-center gap-2"><input className="flex-1 accent-rust" type="range" min={key === "quiet" ? 2 : key === "logoSize" ? 5 : key === "thickness" ? 50 : 0} max={key === "quiet" ? 8 : key === "logoSize" ? 30 : key === "thickness" ? 100 : 40} value={design[key]} onChange={e => update({ [key]: Number(e.target.value) })} /><code className="w-14 text-right text-[10px] font-medium text-muted">{design[key]}{key === "quiet" ? " modules" : "%"}</code></div></label>;
