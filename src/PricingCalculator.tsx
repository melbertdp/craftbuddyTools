          ...
          <div className="grid lg:grid-cols-3">
            <CostColumn title="Materials">
              ...
              {materialRows.map((row) => <div key={row.id} className="grid grid-cols-[minmax(0,1fr)_70px_60px_90px_90px_28px] divide-x-2 divide-[#a9b8a0] border-b border-[#cbd8c3] last:border-0">
                <Input className="h-8 rounded-none border border-[#a9b8a0] bg-white text-xs" placeholder="Material name" value={row.name} onChange={(e) => updateMaterial(row.id, { name: e.target.value })} />
                <Input className="h-8 rounded-none border border-[#a9b8a0] bg-white text-right text-xs" type="number" min="0" step="1" value={row.quantity} onChange={(e) => updateMaterial(row.id, { quantity: e.target.value })} />
                <Input className="h-8 rounded-none border border-[#a9b8a0] bg-white text-xs" value={row.unit} onChange={(e) => updateMaterial(row.id, { unit: e.target.value })} />
            ...
            <CostColumn title="Labor">
              ...
              {laborRows.map((row) => <div key={row.id} className="grid grid-cols-[minmax(0,1fr)_82px_105px_90px_28px] divide-x-2 divide-[#a9b8a0] border-b border-[#cbd8c3] last:border-0">
                <Input className="h-8 rounded-none border border-[#a9b8a0] bg-white text-xs" placeholder="Task description" value={row.description} onChange={(e) => updateLabor(row.id, { description: e.target.value })} />
                <Input className="h-8 rounded-none border border-[#a9b8a0] bg-white text-right text-xs" type="number" min="0" step="1" value={row.minutes} onChange={(e) => updateLabor(row.id, { minutes: e.target.value })} />
            ...
            <CostColumn title="Other">
              ...
              {otherRows.map((row) => <div key={row.id} className="grid grid-cols-[minmax(0,1fr)_70px_90px_90px_28px] divide-x-2 divide-[#a9b8a0] border-b border-[#cbd8c3] last:border-0">
                <Input className="h-8 rounded-none border border-[#a9b8a0] bg-white text-xs" placeholder="e.g. Packaging" value={row.type} onChange={(e) => updateOther(row.id, { type: e.target.value })} />
                <Input className="h-8 rounded-none border border-[#a9b8a0] bg-white text-right text-xs" type="number" min="0" step="1" value={row.quantity} onChange={(e) => updateOther(row.id, { quantity: e.target.value })} />
...
function MoneyInput({ value, onChange }: { value: string; onChange: (value: string) => void }) { return <div className="flex items-center rounded-none border border-[#a9b8a0] bg-white"><span className="pl-2 text-[11px] text-muted-foreground">₱</span><Input className="h-8 rounded-none border-0 text-right text-xs" type="number" min="0" step="0.01" value={value} onChange={(e) => onChange(e.target.value)} /></div>; }
function MoneyValue({ value }: { value: number }) { return <span className="flex h-8 items-center justify-end gap-1 rounded-none border border-[#a9b8a0] bg-[#f4f7f1] px-2 text-xs"><span className="text-[11px] text-muted-foreground">₱</span>{money(value)}</span>; }
